...
if __name__ == '__main__':
    ...
    net = LaneDect()
    state_dict = torch.load("/workspace/LaneDect/model/LaneDect_29.pth")
    ...
    optimizer = torch.optim.Adam(training_params, lr=0.000005, weight_decay=0.0001)

    for epoch in range(30, 50):
        for i, data in enumerate(train_loader):
            net.train()
            img, label_row, label_range, label_hm, mask = data
            pre_seg, pre_range, pre_hm = net(img.to(device))

            ls = loss(pre_seg, pre_range, pre_hm, label_row.to(device).float(),
            label_range.to(device).long(), label_hm.to(device).float(), mask.to(device).float())
            optimizer.zero_grad()
            print(ls)
            ls.backward()
            optimizer.step()
